...
def _resize_for_tg(path: Path):
    try:
        ...
        # Сохраняем пропорции, ограничиваем максимальный размер
...
async def demo_bot(cb: CallbackQuery, state: FSMContext):
    ...
    await edit_or_answer(cb,
        "🤖 <b>Какие боты мы делаем:</b>\n\n"
        "📥 <b>Бот-воронка</b> — собирает заявки, квалифицирует клиентов, отправляет прайс и портфолио\n\n"
        "📅 <b>Бот для записи</b> — клиент сам выбирает удобное время, вы получаете уведомление\n\n"
        "🛒 <b>Бот-магазин</b> — каталог товаров или услуг с приёмом оплаты прямо в Telegram\n\n"
        "💬 <b>Бот-консультант</b> — отвечает на частые вопросы, прогревает и передаёт заявку вам\n\n"
        "📊 <b>Бот для опросов</b> — собирает обратную связь, анкеты, NPS от клиентов\n\n"
